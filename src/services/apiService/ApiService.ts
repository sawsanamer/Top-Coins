...
export const endpoints = {
  ...
  coinPrice: (id: string) =>
    `${process.env.REACT_APP_GECKO_BASE_URL}/simple/price?ids=${id}&vs_currencies=${CURRENCY}&x_cg_demo_api_key=${process.env.REACT_APP_GECKO_API_KEY}`,
};
export const coinKeys = {
  all: ["coins"] as const,
  historicalChart: (coinId: string, days: number) =>
    [...coinKeys.all, "historicalChart", coinId, days] as const,
  coinPrice: (coinId: string) =>
    [...coinKeys.all, "coinPrice", coinId] as const,
};

export class ApiService {
  static async getHistoricalChart(
    coinId: string,
    days: number,
